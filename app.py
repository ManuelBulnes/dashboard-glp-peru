...
col1.metric("Registros", safe(kpis["registros"]), delta=safe(kpis["registros"] - kpis_full["registros"]))
col2.metric("Promedio", round(safe(kpis["precio_promedio"]), 2), delta=round(safe(kpis["precio_promedio"] - kpis_full["precio_promedio"]), 2))
...
with tab3:
    ...
    # MEJORAS VISUALES
    fig_scatter.update_traces(marker=dict(size=11, line=dict(width=1)))
    ...
    if not trend.empty:
        ...
        fig_line = px.line(trend, x="periodo", y="precio_de_venta_(soles)", markers=True)
    ...
    fig_box.update_traces(marker=dict(size=9, opacity=0.7, line=dict(width=1)))
...
with tab5:
    ...
    if not df_p.empty:
        ...
        if fig_radar is not None:
            fig_radar.update_layout(title=f"🛡️ Perfil Analítico del Producto: {producto_sel}")
